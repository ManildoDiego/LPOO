...
void ordenarCanciones(vector<Cancion>& canciones) {
  bool intercambiado = true;
  while (intercambiado) {
    intercambiado = false;
    for (size_t i = 0; i < canciones.size() - 1; i++) {
      if (canciones[i].nombreCompleto > canciones[i + 1].nombreCompleto) {
...
int main() {
  ...
  while (1) {
    ...
    cout << "6. Ordenar canciones por nombre" << endl;
    ...
    switch (opcion) {
      ...
      case 6:
        ...
        cout << "Canciones ordenadas por nombre:" << endl;
        for (const auto& c : canciones) {
          cout << c << endl;
        }
        system("pause");
